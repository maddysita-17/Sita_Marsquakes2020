...
for i in [*range(5)]:
    ...
    samp_errP = np.random.uniform(0,Pst)
    samp_errS = np.random.uniform(0,Sst)

    Pin = P + samp_errP
    SHin = SH + samp_errS
    SVin = SV + samp_errS

    outputf = autofault(dataf, obs_P = Pin, obs_SV = SVin, obs_SH = SHin, errP = samp_errP, errS = samp_errS)
    print('Std: ', samp_errP, samp_errS)
    outputf = outputf.sort_values('Misfit')
    print(outputf.head(10))
    n += 1
