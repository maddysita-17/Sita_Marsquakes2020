...
def autofault(df, obsP, obsSV, obsSH, errP, errSV, errSH):
    ...
    for i in select:
        ...
        azt_ls.append(azt); dpt_ls.append(dpt)
...
def forwardcalc(depth,mechdic,alt=False):
    ...
    if 'S0325ab' in mechdic:
        ...
        print('325aa')
        Pp, Sp, Pa, Sa = eventbuild('325aa', 38.4)
        # data325ab, Pe, Se = getamp(-60.38, [s0325ab[0]], [s0325ab[1]], [s0325ab[2]], [Pp, Sp])
        data325ab, Pe, Se = getamp(-114.25, [s0325ab[0]], [s0325ab[1]], [s0325ab[2]], [Pp, Sp])


        print(data325ab)
        for i in range(len(data325ab)):
            data325ab.at[i,'Model'] = 'InSightMod'
        data325ab.to_csv(path+'/fwdcalc-325aa.csv', mode='a',header=False)
        ...
        #         data1.at[i,'Model'] = 'InSightbAz'
        #     data1.to_csv(path+'/csvs/fwdcalc-325aa.csv', mode='a',header=False)
    else:
        pass
...
path = '/Users/maddysita/Desktop/CIERA_REU/event-by-event/S0325ab'
...
# ------ FAULT PLANE LISTS -------
strike_325aa = [*range(0,360,2)]
dip_325aa = [*range(0,90,2)]
rake_325aa = [*range(-180,180,2)]
...
def eventoutput(depth,rank):
    ...
    data325ab, Pe, Se = getamp(-60.43, strike_325aa, dip_325aa, rake_325aa, [Pp, Sp])
    # print('Exit Angles: ', Pe, Se)
    data325ab = autofault(data325ab,  obsP=5.38e-10, obsSV=-1.44e-9, obsSH=-9.63e-10, errP=1.75e-10, errSV=4.79e-10, errSH=4.25e-10)
    print(len(data325ab))
    print(data325ab[:5])
    data325ab.to_csv(path + '/csvs/our325aa_123_2.csv', mode='a', header=False)

    print('325aa')
    Pp, Sp, Pa, Sa = eventbuild('325a', 38.4)
    data325ab, Pe, Se = getamp(-114.25, strike_325aa, dip_325aa, rake_325aa, [Pp, Sp])
    # print('Exit Angles: ', Pe, Se)
    data325ab = autofault(data325ab,  obsP=5.38e-10, obsSV=0, obsSH=-1.77e-9, errP=1.75e-10, errSV=4.79e-10, errSH=4.25e-10)
    print(len(data325ab))
    print(data325ab[:5])
    data325ab.to_csv(path + '/csvs/insight325aa_57_2.csv', mode='a', header=False)
    return
...
for mod in ['NewGudkova']:
    ...
    for depth in [35]:
        ...
        forwardcalc(depth,mechanism_dict,alt=False)
