...
mechanism_dict = {
                     # 'S0325ab':  [[4,0,70],],          # NewGud Top Mech @ 35km w/ wrong rake range
                     'S0325ab':  [[70,76,-116],],
                    }
...
def forwardcalc(depth,mechdic,alt=False):
    ...
    if 'S0325ab' in mechdic:
        ...
        for i in range(len(data325ab)):
            data325ab.at[i,'Model'] = mod
        data325ab.to_csv(path+'/fwdcalc-325ab.csv', mode='a',header=False)
        ...
        #         data1.at[i,'Model'] = 'ForwardCalc'
        #     data1.to_csv(path+'/csvs/fwdcalc-325ab.csv', mode='a',header=False)
    else:
        pass
...
strike_325ab = [*range(0,360,2)]
dip_325ab = [*range(0,90,2)]
rake_325ab = [*range(-180,180,2)]
...
def eventoutput(depth,rank):
    ...
    data325ab, Pe, Se = getamp(-45.6, strike_325ab, dip_325ab, rake_325ab, [Pp, Sp])
    # print('Exit Angles: ', Pe, Se)
    data325ab = autofault(data325ab,  obsP=-1.35e-9, obsSV=-4.06e-9, obsSH=0.25e-9, errP=1.75e-10, errSV=4.79e-10, errSH=4.25e-10)
    print(len(data325ab))
    print(data325ab[:5])
    data325ab.to_csv(path + '/csvs/ng325ab_fullsyn.csv', mode='a', header=False)
    return
