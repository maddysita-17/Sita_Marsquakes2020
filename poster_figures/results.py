...
def wfplot(st,axs,iax,scale=1,sh=0,Ptime=0):
    ...
    shift = sh
    for trace in sENZ:
        axs[iax].plot(time_axis[:n], a*trace.data[:n] + shift, label = trace.stats.channel)
        shift += dsh
    axs[iax].set_ylim(ylim)
    if iax == len(axs)-1:
       axs[len(axs)-1].legend(loc='lower right', ncol=2, fontsize='x-small', title='Reference  vs   New Event', title_fontsize='x-small')
...
fig, axs = plt.subplots(nrows, 1, figsize=(10,15))
for j in range(nrows):
    axs[j].tick_params(labelsize=6)
    axs[j].set_prop_cycle(color=['#3BA7BF', '#6297A3', '#898686', '#BF533B', '#D8802A', '#F0AC19'], alpha=[0.5,0.5,0.5,1.0,1.0,1.0])
plt.subplots_adjust(left=0.03, right=0.97, bottom=0.03, top=0.95, hspace=0.7)
iax = 0
...
axs[iax].set_title('Raw Data', color='w')
iax += 1
...
axs[iax].set_title('High-Passed Data: 8Hz', color='w')
iax += 1
...
for f in bpfilters:
    ...
    #time_axis = np.arange(start235b-P235b,end235b-P235b+dt,dt)
    wfplot(stm,axs,iax,scale,shift,P235b)
    axs[iax].set_title('Band-Passed Data: ' + str(f[0]) + '-' + str(f[1]) + 'Hz', color='w')
    iax += 1
...
#time_axis = np.arange(start235b-P235b,end235b-P235b+dt,dt)
wfplot(stm,axs,iax,scale,shift,P235b)
axs[iax].set_title('Low-Passed Data: 0.125Hz', color='w')
...
fig.savefig('nez_plot.png', transparent=True)
plt.show()
